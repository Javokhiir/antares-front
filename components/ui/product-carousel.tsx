...
function CarouselIndicator({
  ...
  return (
    <div
      className={cn(
        "absolute top-0 right-0 z-10 flex h-full w-min items-center justify-center",
        className
      )}
    >
      <div className="flex w-min flex-col items-center space-y-2">
        {Array.from({ length: itemsCount }, (_, i) => (
          <button
            ...
            onClick={() => setIndex(i)}
            className={cn(
              "h-2 w-2 cursor-pointer rounded-full transition-opacity duration-300",
              index === i
                ? "bg-primary h-3 w-3 dark:bg-zinc-50"
                : "bg-primary/50 dark:bg-zinc-100/50",
              classNameButton
            )}
          />
        ))}
